     ...
     <form wire:submit.prevent="saveStory" class="flex flex-col gap-5">
                 ...
                 <div class="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                     ...
                     <div class="flex-1">
                         <input type="file" wire:model="cover"
                             class="text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:bg-slate-900 file:text-white hover:file:bg-emerald-600 file:transition-all">
         ...
         <div>
             ...
             <input type="text" wire:model="title" placeholder="Masukkan judul yang menarik..."
                 class="w-full px-4 py-3.5 bg-slate-50 border @error('title') border-rose-500 @else border-slate-100 @enderror rounded-2xl text-sm focus:outline-hidden focus:border-emerald-500 focus:bg-white transition-all shadow-2xs">
             @error('title')
                 <span class="text-[10px] text-rose-500 mt-1 block font-bold px-1">{{ $message }}</span>
             @enderror
         </div>
         ...
         <div>
             ...
             <select wire:model="genreId"
                 class="w-full px-4 py-3.5 bg-slate-50 border @error('genreId') border-rose-500 @else border-slate-100 @enderror rounded-2xl text-sm focus:outline-hidden focus:border-emerald-500 focus:bg-white transition-all shadow-2xs text-slate-700">
         ...
         <div>
             ...
             <textarea wire:model="synopsis" rows="5" placeholder="Tulis blurb atau sinopsis singkat cerita..."
                 class="w-full px-4 py-3.5 bg-slate-50 border @error('synopsis') border-rose-500 @else border-slate-100 @enderror rounded-2xl text-sm focus:outline-hidden focus:border-emerald-500 focus:bg-white transition-all shadow-2xs resize-none"></textarea>
             @error('synopsis')
                 <span class="text-[10px] text-rose-500 mt-1 block font-bold px-1">{{ $message }}</span>
             @enderror
         </div>
         ...
         <button type="submit" wire:loading.attr="disabled"
             class="w-full bg-slate-900 hover:bg-emerald-600 text-white font-bold text-sm py-4 rounded-2xl transition-all duration-300 shadow-lg mt-2 flex items-center justify-center gap-2">
